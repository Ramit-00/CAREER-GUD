  ...
  return (
    <div className="flex flex-col gap-5 rounded-2xl border border-slate-200 bg-white p-6 shadow-xs">
      <div className="flex items-center justify-between">
        <div>
          <h4 className="text-base font-bold text-[#0B2A4A]">Aptitude & Interest Distribution</h4>
          <p className="text-xs text-slate-600 mt-0.5">Multi-stream alignment across higher secondary paths</p>
        </div>
        <span className="rounded-md bg-[#FFF8EE] border border-[#FFD8A8] px-2.5 py-1 text-xs font-bold text-[#994500]">
          5 Core Pillars
        </span>
      </div>
          ...
          return (
            <div key={cat.key} className="flex flex-col gap-1.5">
              <div className="flex justify-between text-xs font-bold">
                <span className="text-slate-950">{cat.label}</span>
                <span className="text-slate-700">{percentage}% Affinity</span>
              </div>
              <div className="h-2.5 w-full overflow-hidden rounded-md bg-slate-200">
                <div
